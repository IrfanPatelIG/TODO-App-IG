...
function App() {
  ...
  const TodoCard = ({Data}) => {
    return <>
      <li className='task-lis'>
        <div className='task-num flex-text-center w-6 h-full pl-2'>{Data.id}</div>
        <div className='task-todo flex-text-center w-full h-full p-1'>{Data.task}</div>
        <div className="actions flex items-center gap-3 h-full p-1 *:w-[30px] *:h-[30px] *:transition-[background-image] *:duration-400 *:ease-out">
  ...
  return (
    <>
      <div className='main-container w-screen h-screen font-arimo overflow-x-hidden box-border'>
        <NavBar />

        <div className="w-[98%] h-[90vh] mx-[1vw] my-[1vh] rounded-lg bg-linear-to-br/decreasing from-indigo-500 to-teal-40">
          <main className="hero-section w-fit p-1.5 m-auto translate-y-[25%] flex flex-col items-center gap-3.5 overflow-hidden *:overflow-hidden">
            <div className="task w-fit h-12 m-auto overflow-hidden flex rounded-[28px]">
              <input ref={inputRef} type="text" name="task" placeholder='What to do?' className='w-lg px-4 py-2 text-white text-[19px] bg-[#1018286f] focus:outline-0' />
              <button className='w-32 prime-btn-og'>Add</button>
            </div>

            <div className="tasks-todo w-[94%] flex flex-col gap-3">
              ...
              <ul role='list' className='tasks-list w-full max-h-[340px] pb-1 flex flex-col items-start gap-2 overflow-y-auto'>
                {
                  taskData.map((taskD) => {
